...
describe('File uploader simple', () => {
  ...
  describe('File Upload', () => {
    test('should auto upload files on select and call asyncFileUpload - image Type', async () => {
    ...
    test('should auto upload files on select and call asyncFileUpload -Doc Type', async () => {
      render(<compositions.BasicFileUploaderSimpleDoc />);
      const selectFilesBtn = screen.getByLabelText('Upload');
      const exampleFile: File = new File(['hello'], 'hello.txt', {
        type: 'text/plain',
      });
      await UserEvent.upload(selectFilesBtn, exampleFile);
      await waitFor(() => expect(asyncFileUpload).toHaveBeenCalled());
